...
export class RequestHelpComponent implements OnInit {
  ...
  constructor(
    ...
    public retMessageService: ReturnMessageService,
  ...
  req_submit() {
    if (this.firstname == "" || this.lastname == "" || this.phone == null || this.phone == null || this.address == "" || this.city == null || this.category == "" || this.detailsEvent == "") {
      ...
      this.count = 0;
      this.messageService.add(this.reqData);
      this.callAgain();
  ...
  callAgain() {
    ...
    this.count++;
    this.locationService.publishLocation(this.reqData, this.count).then(res => { });
    setTimeout(() => this.callAgain(), 1000 * 60 * 10);
  }
